...
const ROOT_URL = UNPKG ? `https://d1ks0e82elkklw.cloudfront.net/` : `https://d1o1ymldlm5b10.cloudfront.net/npm/`;
const loadedTypings = [];
...
const getFileMetaData = (dependency, version, depPath) => {
  if (UNPKG) {
    ...
    return doFetch(`https://d1ks0e82elkklw.cloudfront.net/${dependency}@${version}${usedDepPath}?meta`)
...
export async function fetchAndAddDependencies(
  ...
  try {
    if (loadedTypings.indexOf(dep) === -1) {
      ...
      try {
        await doFetch(`https://d1ks0e82elkklw.cloudfront.net/${dep}@${version}/package.json`)
          .then(x => JSON.parse(x))
          .then(x => {
